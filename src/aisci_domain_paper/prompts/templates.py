...
Use `edit_file` to write or update the analysis files. Do not finish until all required files exist.
"""
...
When model-native web search is enabled, you may use it for library docs or installation guidance.
...
When model-native web search is enabled, you may use it for library errors or runtime diagnostics.
...
When model-native web search is enabled, you may use it for documentation or background research.
...
Use `edit_file` to write or update `/home/agent/plan.md` before finishing.
"""


GENERAL_SYSTEM_PROMPT = f"""You are a General-Purpose Agent for an AI paper reproduction project. You handle auxiliary tasks that require code execution and file modification but do not fit the specialized implement or experiment workflows.
...
Decide:
1. how many searches to run
2. what each search should focus on
3. which files or sections matter most

Return a concise structured plan via `subagent_complete`.
"""
